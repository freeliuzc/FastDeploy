      ...
      if (seq_lens_encoder[tid] > 0) {
        ...
        if (TRCUNCATE_FIRST_TOKEN) {
          ...
          seq_lens_this_time[tid] = seq_len_encoder + 1;
                 ...
                 num_model_step) /*Accept partial draft tokens*/ {
        // Base Model reject stop
        if (stop_flags[tid]) {
          stop_flags[tid] = false;
          seq_lens_decoder[tid] = base_model_seq_lens_decoder[tid] - 1;
          step_idx[tid] = base_model_step_idx[tid] - 1;
        } else {
          seq_lens_decoder[tid] -= num_model_step - accept_num_now;
          step_idx[tid] -= num_model_step - accept_num_now;
        }
        const int64_t modified_token = accept_tokens_now[accept_num_now - 1];
        pre_ids_now[base_model_step_idx[tid] - 1] = modified_token;
        draft_tokens_now[0] = modified_token;
        seq_lens_this_time[tid] = 1;

      } else /*Accept all draft tokens*/ {
        for (int i = num_model_step; i < accept_num_now; i++) {
          const int token_pos = i - num_model_step + 1;
          const int pre_id_pos = base_model_step_idx[tid] - (accept_num_now - i);
          const int64_t accept_token = accept_tokens_now[i];
          draft_tokens_now[token_pos] = accept_token;
          pre_ids_now[pre_id_pos] = accept_token;
        }
        seq_lens_this_time[tid] = accept_num_now - num_model_step + 1;

        // draft_tokens_now[1] = accept_tokens_now[num_model_step];
        // pre_ids_now[base_model_step_idx[tid] - 1] = accept_tokens_now[num_model_step];
